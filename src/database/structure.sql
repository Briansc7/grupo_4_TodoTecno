-- MySQL Script generated by MySQL Workbench
-- Sun Apr 23 19:58:36 2023
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema Todo_Tecno
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `Todo_Tecno` ;

-- -----------------------------------------------------
-- Schema Todo_Tecno
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `Todo_Tecno` DEFAULT CHARACTER SET utf8 ;
USE `Todo_Tecno` ;

-- -----------------------------------------------------
-- Table `Todo_Tecno`.`roles`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`roles` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`contactInformation`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`contactInformation` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `address` VARCHAR(45) NULL,
  `location` VARCHAR(45) NULL,
  `province` VARCHAR(45) NULL,
  `zipCode` VARCHAR(8) NULL,
  `phone` VARCHAR(15) NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`users`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`users` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `firstName` VARCHAR(45) NOT NULL,
  `lastName` VARCHAR(45) NOT NULL,
  `email` VARCHAR(45) NOT NULL,
  `password` VARCHAR(60) NOT NULL,
  `contactInfoId` INT(10) UNSIGNED NULL,
  `birthday` DATE NULL,
  `image` VARCHAR(45) NULL,
  `roleId` INT(10) UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `email_UNIQUE` (`email` ASC) VISIBLE,
  INDEX `firstName` (`firstName` ASC) VISIBLE,
  INDEX `lasName` (`lastName` ASC) VISIBLE,
  INDEX `role_idx` (`roleId` ASC) VISIBLE,
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `userAddress_idx` (`contactInfoId` ASC) VISIBLE,
  CONSTRAINT `role`
    FOREIGN KEY (`roleId`)
    REFERENCES `Todo_Tecno`.`roles` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `userContactInfo`
    FOREIGN KEY (`contactInfoId`)
    REFERENCES `Todo_Tecno`.`contactInformation` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`categories`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`categories` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE);


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`subCategories`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`subCategories` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `categoryId` INT(10) UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `category_idx` (`categoryId` ASC) VISIBLE,
  CONSTRAINT `category`
    FOREIGN KEY (`categoryId`)
    REFERENCES `Todo_Tecno`.`categories` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`brands`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`brands` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`products`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`products` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `subCategoryId` INT(10) UNSIGNED NOT NULL,
  `brandId` INT(10) UNSIGNED NOT NULL,
  `model` VARCHAR(45) NOT NULL,
  `artNumber` BIGINT UNSIGNED NULL,
  `price` DECIMAL(10,2) UNSIGNED NOT NULL,
  `discountPorc` TINYINT UNSIGNED NULL DEFAULT 0,
  `isOnSale` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0,
  `isNew` TINYINT(1) UNSIGNED NOT NULL DEFAULT 1,
  `description` TEXT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `subCategory_idx` (`subCategoryId` ASC) VISIBLE,
  INDEX `brand_idx` (`brandId` ASC) VISIBLE,
  UNIQUE INDEX `artNumber_UNIQUE` (`artNumber` ASC) VISIBLE,
  CONSTRAINT `subCategory`
    FOREIGN KEY (`subCategoryId`)
    REFERENCES `Todo_Tecno`.`subCategories` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `brand`
    FOREIGN KEY (`brandId`)
    REFERENCES `Todo_Tecno`.`brands` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`stores`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`stores` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `contactInfoId` INT(10) UNSIGNED NOT NULL,
  `description` TEXT NULL,
  `shoppingHours` VARCHAR(60) NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `storeAddress_idx` (`contactInfoId` ASC) VISIBLE,
  CONSTRAINT `storeContactInfo`
    FOREIGN KEY (`contactInfoId`)
    REFERENCES `Todo_Tecno`.`contactInformation` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`statusStock`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`statusStock` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`stock`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`stock` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `productId` INT(10) UNSIGNED NOT NULL,
  `storeId` INT(10) UNSIGNED NOT NULL,
  `quantity` MEDIUMINT UNSIGNED NOT NULL,
  `statusId` INT(10) UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `product_idx` (`productId` ASC) VISIBLE,
  INDEX `store_idx` (`storeId` ASC) VISIBLE,
  INDEX `status_idx` (`statusId` ASC) VISIBLE,
  CONSTRAINT `productOfStock`
    FOREIGN KEY (`productId`)
    REFERENCES `Todo_Tecno`.`products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `storeOfStock`
    FOREIGN KEY (`storeId`)
    REFERENCES `Todo_Tecno`.`stores` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `status`
    FOREIGN KEY (`statusId`)
    REFERENCES `Todo_Tecno`.`statusStock` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`discountCupons`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`discountCupons` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `cuponCode` VARCHAR(45) NOT NULL,
  `startDate` DATE NULL,
  `endDate` DATE NULL,
  `discountPorcentage` TINYINT UNSIGNED NOT NULL,
  `minPurchaseRequired` DECIMAL(10,2) UNSIGNED NULL,
  `maxDiscountValue` DECIMAL(10,2) UNSIGNED NULL,
  `maxUsesPerUser` SMALLINT UNSIGNED NOT NULL DEFAULT 1,
  `maxUsesTotal` INT UNSIGNED NULL,
  `usesCount` INT UNSIGNED NOT NULL DEFAULT 0,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  UNIQUE INDEX `cuponCode_UNIQUE` (`cuponCode` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`statusSales`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`statusSales` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`InterestOptions`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`InterestOptions` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `installmentsQuantity` TINYINT UNSIGNED NOT NULL,
  `interestPorcentage` TINYINT UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`paymentMethods`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`paymentMethods` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(60) NOT NULL,
  `interestOption` INT(10) UNSIGNED NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `interestOption_idx` (`interestOption` ASC) VISIBLE,
  CONSTRAINT `interestOption`
    FOREIGN KEY (`interestOption`)
    REFERENCES `Todo_Tecno`.`InterestOptions` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`sales`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`sales` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `userId` INT(10) UNSIGNED NOT NULL,
  `chargeProducts` DECIMAL(10,2) UNSIGNED NOT NULL DEFAULT 0,
  `chargeDelivery` DECIMAL(10,2) UNSIGNED NOT NULL DEFAULT 0,
  `discountCuponAmount` DECIMAL(10,2) UNSIGNED NOT NULL DEFAULT 0,
  `chargeTotalSale` DECIMAL(10,2) GENERATED ALWAYS AS (chargeproducts+chargedelivery-discountcuponamount) VIRTUAL,
  `discountCuponId` INT(10) UNSIGNED NULL,
  `date` DATE NOT NULL,
  `storeId` INT(10) UNSIGNED NOT NULL,
  `paymentMethodId` INT(10) UNSIGNED NOT NULL,
  `isPickup` TINYINT(1) UNSIGNED NOT NULL DEFAULT 1,
  `statusId` INT(10) UNSIGNED NOT NULL,
  `contactInfoId` INT(10) UNSIGNED NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `user_idx` (`userId` ASC) VISIBLE,
  INDEX `discountCupon_idx` (`discountCuponId` ASC) VISIBLE,
  INDEX `store_idx` (`storeId` ASC) VISIBLE,
  INDEX `status_idx` (`statusId` ASC) VISIBLE,
  INDEX `paymentMethod_idx` (`paymentMethodId` ASC) VISIBLE,
  INDEX `saleContactInformation_idx` (`contactInfoId` ASC) VISIBLE,
  CONSTRAINT `user`
    FOREIGN KEY (`userId`)
    REFERENCES `Todo_Tecno`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `discountCupon`
    FOREIGN KEY (`discountCuponId`)
    REFERENCES `Todo_Tecno`.`discountCupons` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `storeOfSale`
    FOREIGN KEY (`storeId`)
    REFERENCES `Todo_Tecno`.`stores` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `statusOfSale`
    FOREIGN KEY (`statusId`)
    REFERENCES `Todo_Tecno`.`statusSales` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `paymentMethod`
    FOREIGN KEY (`paymentMethodId`)
    REFERENCES `Todo_Tecno`.`paymentMethods` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `saleContactInformation`
    FOREIGN KEY (`contactInfoId`)
    REFERENCES `Todo_Tecno`.`contactInformation` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`detailSales`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`detailSales` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `saleId` INT(10) UNSIGNED NOT NULL,
  `productId` INT(10) UNSIGNED NOT NULL,
  `quantity` MEDIUMINT UNSIGNED NOT NULL DEFAULT 0,
  `unitPrice` DECIMAL(10,2) UNSIGNED NOT NULL DEFAULT 0,
  `discountPorc` TINYINT UNSIGNED NOT NULL DEFAULT 0,
  `chargeTotal` DECIMAL(10,2) GENERATED ALWAYS AS (quantity*unitprice*(100 - discountporc)/100) VIRTUAL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `sale_idx` (`saleId` ASC) VISIBLE,
  INDEX `product_idx` (`productId` ASC) VISIBLE,
  CONSTRAINT `sale`
    FOREIGN KEY (`saleId`)
    REFERENCES `Todo_Tecno`.`sales` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `productSold`
    FOREIGN KEY (`productId`)
    REFERENCES `Todo_Tecno`.`products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`productImages`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`productImages` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `fileName` VARCHAR(45) NOT NULL,
  `productId` INT(10) UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `product_idx` (`productId` ASC) VISIBLE,
  CONSTRAINT `productOfImages`
    FOREIGN KEY (`productId`)
    REFERENCES `Todo_Tecno`.`products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`characteristics`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`characteristics` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `productId` INT(10) UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `product_idx` (`productId` ASC) VISIBLE,
  CONSTRAINT `productOfCharacteristic`
    FOREIGN KEY (`productId`)
    REFERENCES `Todo_Tecno`.`products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`subCharacteristics`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`subCharacteristics` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `characteristicId` INT(10) UNSIGNED NOT NULL,
  `value` VARCHAR(45) NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `charasteristic_idx` (`characteristicId` ASC) VISIBLE,
  CONSTRAINT `charasteristic`
    FOREIGN KEY (`characteristicId`)
    REFERENCES `Todo_Tecno`.`characteristics` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`boughtTogether`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`boughtTogether` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `product1` INT(10) UNSIGNED NOT NULL,
  `product2` INT(10) UNSIGNED NOT NULL,
  `timesBoughtTogether` INT UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `productA_idx` (`product1` ASC) VISIBLE,
  INDEX `productB_idx` (`product2` ASC) VISIBLE,
  CONSTRAINT `productA`
    FOREIGN KEY (`product1`)
    REFERENCES `Todo_Tecno`.`products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `productB`
    FOREIGN KEY (`product2`)
    REFERENCES `Todo_Tecno`.`products` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`cuponsUsedByUsers`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`cuponsUsedByUsers` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `userId` INT(10) UNSIGNED NOT NULL,
  `cuponId` INT(10) UNSIGNED NOT NULL,
  `usesCount` SMALLINT UNSIGNED NOT NULL DEFAULT 0,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `whoUsedCupon_idx` (`userId` ASC) VISIBLE,
  INDEX `cuponUsed_idx` (`cuponId` ASC) VISIBLE,
  CONSTRAINT `whoUsedCupon`
    FOREIGN KEY (`userId`)
    REFERENCES `Todo_Tecno`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `cuponUsed`
    FOREIGN KEY (`cuponId`)
    REFERENCES `Todo_Tecno`.`discountCupons` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `Todo_Tecno`.`brandsOfCategories`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `Todo_Tecno`.`brandsOfCategories` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `brandId` INT(10) UNSIGNED NOT NULL,
  `categoryId` INT(10) UNSIGNED NOT NULL,
  `createdAt` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `updatedAt` TIMESTAMP NULL,
  `deletedAt` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `brandOfCategory_idx` (`brandId` ASC) VISIBLE,
  INDEX `categoryOfBrand_idx` (`categoryId` ASC) VISIBLE,
  CONSTRAINT `brandOfCategory`
    FOREIGN KEY (`brandId`)
    REFERENCES `Todo_Tecno`.`brands` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `categoryOfBrand`
    FOREIGN KEY (`categoryId`)
    REFERENCES `Todo_Tecno`.`categories` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
